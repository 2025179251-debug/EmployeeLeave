...
public class ApplyLeaveServlet extends HttpServlet {
    ...
    private int getStatusId(Connection con, String statusCode) throws SQLException {
        ...
        throw new SQLException("STATUS_CODE not found in LEAVE_STATUSES: " + statusCode);
    }

    private void ensureBalanceRow(Connection con, int empId, int leaveTypeId) throws SQLException {
        // LEAVE_BALANCES columns based on your screenshot:
        // BALANCE_ID (seq default), EMPID, LEAVE_TYPE_ID, ENTITLEMENT, CARRIED_FWD, USED, PENDING, TOTAL
        String merge =
    ...
    private void addPendingAndRecalcTotal(Connection con, int empId, int leaveTypeId, double pendingDays) throws SQLException {
        ...
        // When we add pending, compute TOTAL based on new pending.
        String upd =
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // ✅ your schema: LEAVE_TYPE_ID, TYPE_CODE, DESCRIPTION
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int leaveTypeId;
        try {
            leaveTypeId = Integer.parseInt(leaveTypeIdStr);
        ...
        String halfSession = null; // if you want to store it in DB column HALF_SESSION
        ...
        // ✅ DB constraint expects: DURATION in ('FULL_DAY','HALF_DAY')
        ...
        try {
            ...
            // 1) INSERT LEAVE_REQUESTS (DO NOT insert LEAVE_ID manually!)
            //    This avoids ORA-32795 if LEAVE_ID is identity/generated.
            String insertLeave =
            ...
            try (PreparedStatement ps = con.prepareStatement(insertLeave, new String[]{"LEAVE_ID"})) {
                ...
                ps.setString(6, durationDb);      // FULL_DAY / HALF_DAY
                ps.setDouble(7, durationDays);    // 0.5 / N
                ps.setString(8, reason.trim());
                ps.setString(9, halfSession);     // AM/PM or null

                ps.executeUpdate();

                try (ResultSet keys = ps.getGeneratedKeys()) {
                    if (keys.next()) {
                        leaveId = keys.getInt(1);
                    } else {
                        throw new SQLException("Failed to retrieve generated LEAVE_ID.");
                    }
                }
            }

            // 2) INSERT attachment (if any)
            if (hasFile) {
                String insertFile =
            ...
            // 3) OPTION 2: Update LEAVE_BALANCES (pending naik)
